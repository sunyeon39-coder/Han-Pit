...
function resolveNotificationTargetUrl(raw) {
  const rel = String(raw || "").trim() || "./global-layout.html";
...
function showSeatNotificationFromPayload(data = {}, titleOverride = "") {
  ...
  const targetUrl = resolveNotificationTargetUrl(data.targetUrl || "./global-layout.html");
  const notifyTag =
...
self.addEventListener("notificationclick", (event) => {
  ...
  const targetUrl = resolveNotificationTargetUrl(
    event.notification?.data?.targetUrl || "./global-layout.html"
  );
